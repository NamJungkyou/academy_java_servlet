...
3. \${stars[2]} = ${stars[2]} 
// 다른 내장객체에 동일한 이름(stars)가 attribute로 추가된 것이 없을 때만 requestScope을 생략 가능
4. \${stars["3"]} = ${stars["3"]} 
5. \${requestScope["stars"]["4"]} = ${requestScope["stars"]["4"]}
</pre>
<%
	// 맵 객체를 생성
	java.util.Map<String, String> starMap = new java.util.HashMap<String, String>();
	starMap.put("S1", "화사");
	starMap.put("S2", "슬래쉬");
	starMap.put("S3", "로니 제임스 디오");
	starMap.put("S4", "최민식");
	starMap.put("S5", "박보영");
...
2. \${starMap["S3"]} = ${starMap["S3"]}
2. \${starMap["S4"]} = ${starMap["S4"]}
2. \${starMap["S5"]} = ${starMap["S5"]}
</pre>
<hr>

<h3>scriptle, expression Tag로 목록 추출시</h3>
<%
...
<h3>scriptlet, expression Tag로 맵 추출시</h3>
