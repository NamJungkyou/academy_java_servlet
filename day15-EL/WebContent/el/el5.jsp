...
<body>
	...
	2. request : 응답(response)이 일어나기 전까지 유지되는 객체.
				...
				응답이 일어나기 전까지는 해당 request에 설정(추가) 된 attribute가 유지된다.
	...
	4. application : 하나의 웹 어플리케이션당 1개씩만 생성되는 객체.
					웹 컨테이너에 하나의 서비스가 배포되어서 유지되는동안 계속하여 유지된다.
					서버가 종료될 때 까지 계속해서 하나의 객체가 유지됨.
					즉 유지범위가 가장 길다.
	* 내장객체의 유지 범위
	page => request => session => application
	
	EL의 내장객체
	pageScope => requestSope => sessionScope => applicationScope
	</pre>
...
<%
	...
	request.setAttribute("name", "길동이-session");
...
%>
1. page Id = <%=pageId%> <br>
2. request Id = <%=reqId%> <br>
3. session Id = <%=sessId%> <br>
4. application Id = <%=appId%> <br>

request Name = <%=reqName%> <br>
session Name = <%=sessName%> <br>
</body>
</html>