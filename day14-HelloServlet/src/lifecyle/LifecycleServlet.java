...
/**
 * 서블릿의 라이프사이클(생명주기) 메소드인 init(), service(), destroy()를 테스트하는 서블릿
 * @author Byun
 *
 */
...
public class LifecycleServlet extends HttpServlet {
	...
	/**
	 * 웹 컨테이너에(tomcat)에 의해 서블릿 초기화 과정에서 최초로 1회만 호출되는 라이프 사이클 메소드
	 * 
	 * 파라미터로 전달되는 config는 웹 컨테이너에의해 자동으로 생성되어 전달된다.
	 */
	...
	/**
	 * 웹 컨테이너(tomcat)에 의해 서비스 종료 과정에서 마지막에 1회만 호출되는 라이프 사이클 메소드
	 */
	public void destroy() {
